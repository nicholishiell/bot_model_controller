...
int main(int argc,char* argv[]){  
  ...
  frontSensorPublisherRequest.request.topicName = "rearSensorData"+randId; 
  frontSensorPublisherRequest.request.queueSize = 1; 
  frontSensorPublisherRequest.request.streamCmd = simros_strmcmd_read_proximity_sensor; 
  frontSensorPublisherRequest.request.auxInt1 = rearSensorHandle; 
  ...
  while (ros::ok() and simulationRunning){
    ...
    // Set Behaviour booleans based on sensor booleans.
    // If a timedManuver is being executed then don't update.
    if( not timedManuver){
      randomWalk = (holdingPuck and not seeGoal) or
	(not holdingPuck and not seePuck);
      
      trackingPuck = (not holdingPuck and seePuck);
      
      trackingGoal = (holdingPuck and seeGoal and not atGoal);
      
      depositPuck = (holdingPuck and atGoal and seeGoal);
      
      closeServo = (holdingPuck and servoOpen);
      
      openServo = (not holdingPuck and not servoOpen);

      rearEvade = rearSensor and not movingForward;

      frontEvade = frontSensor and movingForward;;
	...
	CloseServo(servoMotorHandle, servoPublisher);
      ...
      if(not holdingPuck && not servoOpen)
